    ...
    public abstract class NPCBehaviorOverride
    {
        ...
        internal static void LoadAll()
        {
            ...
            foreach (Type type in EternityUtils.GetEveryTypeDerivedFrom(typeof(NPCBehaviorOverride), typeof(EternityMod).Assembly))
            {
                ...
                var hasPreAI = false;
                var preAIMethod = type.GetMethod("PreAI", EternityUtils.UniversalBindingFlags);
                if (preAIMethod is not null && preAIMethod.DeclaringType != typeof(NPCBehaviorOverride))
                    hasPreAI = true;

                var hasFindFrame = false;
                var findFrameMethod = type.GetMethod("FindFrame", EternityUtils.UniversalBindingFlags);
        ...
        {
            for (var i = 0; i < BehaviorOverrideSet.Length; i++)
            {
                ...
                if (!Main.dedServ && EternityMod.Instance.PhaseIndicator != null && phaseThresholds.Length >= 1)
                {
                    foreach (var lifeRatio in phaseThresholds)
                        EternityMod.Instance.PhaseIndicator.Call(0, container.BehaviorOverride.NPCOverrideType, (NPC npc, float difficulty) => lifeRatio);
                }
        ...
        #endregion Abstracts and Virtuals
    }
}
